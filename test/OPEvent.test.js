...
const PriceAggregatorsContract = artifacts.require('PriceAggregators');
...
contract("OPEvent", async (accounts) => {
    ...
    before( async () => {
        ...
        contracts['PriceAggregators'] = await PriceAggregatorsContract.new();
        Object.keys(contracts).forEach((key) => {
            console.log(key + " address:" + contracts[key].address)
        })
        await mintTokensToAddresses(contracts, accounts);
        [OPEvent, OToken, IOToken] = await deployContract(contracts, accounts);
    })
